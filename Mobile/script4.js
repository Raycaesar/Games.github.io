...
class InputHandler{
	constructor(){
		this.keys = [];
		this.touchY = '';
		this.touchTreshold = 100; //touch times
...
class Player{
	...
	draw(context){
		...
		context.beginPath();
		context.arc(this.x + this.width/2 + 20, this.y+this.height/2 + 20, this.width/3, 0, Math.PI*2); //how to make the circle smaller?
		context.stroke();*/

		context.drawImage(this.image, this.frameX*this.width, this.frameY*this.height, this.width, this.height, this.x, this.y, this.width, this.height);
	}
	//For resize context.arc(this.x + this.width/2-20, this.y+this.height/2, this.width/2, 0, Math.PI*2);
	update(input, deltaTime, enemies){
		//collision detection
		enemies.forEach(enemy => {
			const dx = (enemy.x + enemy.width/2 -20) - (this.x + this.width/2 +20) ; // 
			const dy = (enemy.y + enemy.height/2 + 20) - (this.y + this.height/2) ; // must be same as line 86
			const distance = Math.sqrt(dx*dx + dy*dy);
			if (distance < enemy.width/3 + this.width/3 ){ //same as above width/x
				gameover = true;
			}
		})
		//sprite animation
...
function toggleFullScreen(){
	...
	if (!document.fullscreenElement){
		...
		}); //requestFullscreen() method is a asynchronous, return a Promise
	} else {
