...
class Game{
	constructor(ctx, width, height){
		...
		this.enemyTimer = 0;
		this.enemyTypes = ['worm', 'ghost', 'spider'] //To push the two enemies together later//
	}
	update(deltaTime){
...
class Enemy{
	...
	class Ghost extends Enemy{
		...
		update(deltaTime){
			...
			this.y += Math.sin(this.angle) * 3//this.curve  use  times makes high wave //;
			this.angle += 0.04; //make the ghosts up and down//
		}
		draw(ctx){
			ctx.save();
			ctx.globalAlpha = 0.7; //make ghosts transparent//
			super.draw(ctx);
			ctx.restore();
		}
	}
		...
		constructor(game){
			...
			this.width = this.spriteWidth/2;
			this.height = this.spriteHeight/2;
			this.x = Math.random() * this.game.width; //start at a random width//
			this.y = 0 - this.height; //Always start at above the ceiling//
			this.image = spider;
			this.vx = 0; //Only move up and down//
			this.vy = 0.2; //Use Math.random() * 0.1 + 0.1 give different spiders different speeds//
			this.maxLength = Math.random() * this.game.height;
		}
		update(deltaTime){
			super.update(deltaTime);
			this.y += this.vy * deltaTime;
			if(this.y > this.maxLength) this.vy *= -1; //Make spiders go up as well//
		}
