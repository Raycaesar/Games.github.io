...
class Enemy{
	...
	class Ghost extends Enemy{
		constructor(game){
			...
			this.angle = 0;	
			this.curve = Math.random() * 5//; give each ghost random wave//
		}
		update(deltaTime){
			super.update(deltaTime);
			this.y += Math.sin(this.angle) * this.curve  //use  times makes high wave //;
			this.angle += 0.06; //make the ghosts up and down//
		}
		draw(){
			ctx.save();
